...
func Run(ctx context.Context, root string, definition *Definition, timeout time.Duration) (RunResult, error) {
	if definition == nil {
	...
	for _, command := range definition.Commands {
		before, stateErr := CaptureRepositoryState(root)
		if stateErr != nil {
		...
		err := process.Run()
		after, afterErr := CaptureRepositoryState(root)
		var changed []string
		if afterErr != nil {
